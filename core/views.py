    ...
    def get_context_data(self, **kwargs):
        ...
        for div in Division.objects.all():
            context['player_stat_list'][str(div)] = Player.objects.filter(roster__team__division=div).values(
                    ...
                    sum_goals=Sum(
                        Case(
                            When(stat__team=F('roster__team'), stat__team__is_active=True, then=Coalesce('stat__goals',0)),
                            default=0,
                    ...
                    sum_assists=Sum(
                        Case(
                            When(stat__team=F('roster__team'), stat__team__is_active=True, then=Coalesce('stat__assists',0)),
                    ...
                    total_points=Sum(
                        Case(
                            When(stat__team=F('roster__team'), stat__team__is_active=True, then=Coalesce('stat__assists', 0)+Coalesce('stat__goals',0)),
                            default=0,
                            output_field=IntegerField(),
                        )
                    ),
                    sum_empty_net=Sum(
                        Case(
                            When(stat__team=F('roster__team'), stat__team__is_active=True, then=Coalesce('stat__empty_net', 0)),
                            default=0,
                            output_field=IntegerField(),
                        )
                    ),
                    sum_goals_against=Sum(
                        Case(
                            When(stat__team=F('roster__team'), stat__team__is_active=True, then=Coalesce('stat__goals_against', 0)),
                            default=0,
                            output_field=IntegerField(),
                        )
                    ),
